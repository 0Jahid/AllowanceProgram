            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_array($ret, MYSQLI_ASSOC)) { ?>
                        ...
                        ?>
                        <td>
                            <form style="display:inline;" action="approveuser.php" method=GET>
                                <input type=text name="nid" value="<?php echo "{$row['nid']}" ?>" hidden></input>
                                <button type="submit" class="btn btn-success btn-sm mr-2">Approve <i class="fa fa-check"></i></button>
                            </form>
                            <form style="display:inline;" action="declineuser.php" method=GET>
                                <input type=text name="nid" value="<?php echo "{$row['nid']}" ?>" hidden></input>
                                <button type="submit" class="btn btn-danger btn-sm">Decline <i class="fa fa-trash"></i></button>
                            </form>
                        </td>
                        </tr>
                    <?php } ?>
